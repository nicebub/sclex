...
/*    ta = NULL;*/
	...
	*/
/*
    if(!isEOF(&parser->lexer) && !matchToken(&parser->lexer,tokenForType(SECTION_STARTER)).lexeme){
	   getNextChar(&parser->lexer);
	   if(!isEOF(&parser->lexer)){
    ...
    if(!matchToken(&parser->lexer,tokenForType(SECTION_STARTER)).lexeme){
	   ...
	   temp = parseRegularExpression(parser);
    ...
    parser->parseTree->finalpos[parser->parseTree->num_re] =temp2->uniq;
    ...
    while(parser->lexer.current_char != EOF && parser->lexer.current_char != '%'){
	   ...
	   pos(&temp3,1);
	   pos(&temp3,0);
	   parser->parseTree->t[ct] = temp3;
	   parser->parseTree->used++;
	   parser->parseTree->finalpos[parser->parseTree->num_re] = temp4->uniq;
	   ct++;
	   parser->parseTree->num_re++;
