...
Functionality: Memory Cleanup before program termination. Anything the main
	driver function creates is released as well as any othe live structure
...
Functionality: takes in the specification file, opens it and then starts
	to parse and seperate the individual comoponents of the file. (The format
	of the file is shown below.) After creating a parse tree of the input,
	...
	lexer for this specific DFA.
		The code associated with each regular expression will be run when
	the attached lexer is compiled and run on an input stream and a token
	is recognized that matches one of the regular expressions constructed.
...
/*  lexfile structure to hold FILE pointer to .l specification file that will be used*/
...
/* 
		Check to see if the parse tree is constructed and if the firstpos
	sets are constructed for the tree, if not then cleanly exit and do 
	nothing else.
		 If they are, then generate the DFA and output it to the new lexer
		file.
	lexfile.tree holds the parse tree and lexfile.fpos holds the firstpos sets
*/	
